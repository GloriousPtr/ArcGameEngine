...
namespace ArcEngine
{
	...
	public:
		...
		virtual void RegisterCB(uint32_t crc, uint32_t size, uint32_t count = 1) override;
		virtual void RegisterSB(uint32_t crc, uint32_t stride, uint32_t count) override;

		virtual void BindCB(GraphicsCommandList commandList, uint32_t crc, uint32_t index = 0) override;
		...
		virtual void SetCBData(GraphicsCommandList commandList, uint32_t crc, const void* data, uint32_t size, uint32_t index = 0) override;
	...
	private:
		...
		eastl::vector_map<uint32_t, eastl::vector<ConstantBuffer>>		m_ConstantBufferMap;
